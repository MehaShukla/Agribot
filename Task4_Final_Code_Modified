    ...
    for cnt in contours:                #for loop is for iterating over all the contours that needs to be drawn
        ...
        if area > 600:                   #this is for filtering the noise ---> that means if the area is >600 then it means we are looking at something substantial and we need to draw contour on that and broadcast the TF for that
            ...
            if midX != 0 and midY != 0 and midX <= 640 and midY <= 480:     #as the frame of the output image is 640 x 480 pixels so this is to prevent accessing values that are out of bound by any case
                ...
                if depth <= 0.80:      #over here the depth value that allows us to pick the tomatoes on the side where the bot is standing is 0.80
...
def moveBot():                      #this function is to move the bot with a particular velocity
    ...
    if motionStage[0] is True:
        if pose[1] >= -0.56:
            ...
            linear_vel = 0.5               #Intial velocity is 0.5
            angular_vel = 0.0 
        ...
        if pose[1] >= 7.22:
            ...
            if flagDetect is True:      #the flagDetect is used in this way that whenever a tomato is visible the speed of the bot is reduced to 0.02 so that the depth value of the tomato can be accessed correctly
                linear_vel = 0.02
                angular_vel = 0.0 
                Pcontroller()
            else:
                linear_vel = 0.2       #if the bot is moving in between the rows but no tomato is visible in the frame of the bot then it will move with a speed of 0.2
                angular_vel = 0.0
        ...
        if pose[2] <= 0:
            ...
            if pose[1] <=8.27:      #pose[1] ---> is the y value of the bot
                ...
                linear_vel = 0.0
                angular_vel = 1.0           #while turning the angular velocity of the bot is 1.0
...
def Pcontroller():     #function for the proportional controller --> this has been used so that the bot is able to correct its path if it gets deflected form its path
    ...
    angular_vel = kp * (regions['bleft']-0.70) #when the left value of the distance as calculated by the laser is 0.70 then the angular_vel is 0 and any deflection from this value will set a certain angular_vel to correct that deflection
