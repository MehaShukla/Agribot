...
def getContours(img):           #this function is used to draw contours in their appropriate places
    ...
    for cnt in contours:                #for loop is for iterating over all the contours that needs to be drawn
        ...
        if area > 800:                   #this is for filtering the noise ---> that means if the area is >600 then it means we are looking at something substantial and we need to draw contour on that and broadcast the TF for that
